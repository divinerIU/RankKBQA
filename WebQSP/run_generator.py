...
from components.gen_dataset_manager import load_and_cache_gen_examples, load_and_cache_reverse_examples, ListDataset
...
def run_prediction(args, dataset, model, tokenizer, output_prediction=False):
    ...
    eval_dataloader = DataLoader(dataset, sampler=eval_sampler, batch_size=args.per_device_eval_batch_size,
    ...
    for feat, pred in tqdm(zip(dataset, all_predictions), total=len(all_predictions), desc='Decoding'):
        ...
        ex_cnt += (decoded_pred[0] == ex.gt.mask_expr)
...
def main():
    ...
    if training_args.do_train:
        train_dataset = ListDataset(load_and_cache_gen_examples(model_args, tokenizer, evaluate=False))
    else:
        train_dataset = ListDataset([])
    if training_args.do_eval:
        eval_dataset = ListDataset(load_and_cache_gen_examples(model_args, tokenizer, evaluate=True))
    else:
