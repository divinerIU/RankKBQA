...
from components.util import dump_json, load_json
...
def _parse_args():
    ...
    parser.add_argument('--pred_file',
                        default='Reading/LLaMA2-13b/CWQ_Freebase_NQ_lora_epoch10/evaluation_beam/beam_test_top_k_predictions.json',
...
def date_post_process(date_string):
    """
    When querying KB, (our) KB tends to autoComplete a date
    e.g.
        - 1996 --> 1996-01-01
        - 1906-04-18 05:12:00 --> 1906-04-18
        - 2005-01-01 00:00:00 --> 2005
        - 1921-09-01 00:00:00 --> 1921-09
    """
    pattern_year_month_date = r"^\d{4}-\d{2}-\d{2}$"
    ...
    if re.match(pattern_year_month_date_moment, date_string):
        if date_string.endswith('00:00:00'):
            ...
            if date_string.endswith('-01-01'):
                date_string = date_string.split('-')[0]
            # 如果日期部分是 -01，去掉 -01，只保留 YYYY-MM
            elif date_string.endswith('-01'):
...
def aggressive_top_k_eval_new(split, predict_file, dataset):
    ...
    if dataset == "CWQ":
        ...
        test_gen_dataset = load_json('data/CWQ/generation/merged/CWQ_sample_test.json')
