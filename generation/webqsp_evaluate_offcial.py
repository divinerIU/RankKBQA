...
def webqsp_evaluate_valid_results(args):
    if args.split == 'dev':
        ...
        res = main(args.pred_file, f'data/WebQSP/origin/WebQSP.sample.{args.split}.json')
...
def CalculatePRF1(goldAnswerList, predAnswerList):
    if len(goldAnswerList) == 0:
        if len(predAnswerList) == 0:
            return [1.0, 1.0, 1.0,
                    1]  # consider it 'correct' when there is no labeled answer, and also no predicted answer
        else:
            return [0.0, 1.0, 0.0,
                    1]  # precision=0 and recall=1 when there is no labeled answer, but has some predicted answer(s)
...
def main(pred_data, dataset_data):
    ...
    print("F1 of average recall and average precision: %.3f" % (
