...
import argparse
import json
from components.util import dump_json
...
def prepare_dataloader(args):
    ...
    with open(args.data_file_name, 'r', encoding='utf-8') as f:
        ...
        data = [json.loads(line) for line in f if print(line) or True]
...
def _parse_args():
    ...
    parser.add_argument('--data_file_name', default='results/gen/WebQSP_test/evaluation_beam_llama/generated_predictions.jsonl')
...
def run_prediction(args, dataloader, output_dir, output_predictions=True):
    ...
    for i, pred in enumerate(dataloader):
        ...
        expressions = [predict for predict in predictions]

        output_list.append({
            'predictions': expressions,
            'gen_label': gen_label,
        })
        if expressions[0].lower() == gen_label.lower():
            ex_cnt += 1

        if any([x.lower() == gen_label.lower() for x in expressions]):
            contains_ex_cnt += 1
