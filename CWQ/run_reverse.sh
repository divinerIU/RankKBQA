...
if [ "$ACTION" = "train" ]; then
    ...
    nohup python -u CWQ/run_reverser.py \
        --dataset ${dataset} \
    ...
    model=$2
    split=${3:-dev}
    log="results/reverse/${dataset}_sample_${split}/evaluation_beam_chatglm_w_bc_flan-t5"
    mkdir -p ${log}
    nohup python -u CWQ/run_reverser.py \
        --dataset ${dataset} \
        --model_type sample_chatglm \
        --model_name_or_path ${model} \
        --eval_beams 15 \
        --do_predict \
        --predict_file results/gen/CWQ_sample_test/evaluation_beam_chatglm/beam_test_top_k_predictions.json \
        --overwrite_output_dir \
        --output_dir ${log} \
